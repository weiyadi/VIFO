...
__all__ = ['AlexNet', 'MEDMNIST_CNN', 'UCI_MLP', 'AlexNetDrop']
...
def make_cifar_alexnet_dropout(num_classes, *args, **kwargs):
    act = torch.nn.SiLU
    p = kwargs.get('p', 0.1)
    return nn.Sequential(
        nn.Conv2d(3, 64, kernel_size=3, padding=1),
...
class AlexNetDrop:
    base = make_cifar_alexnet_dropout
    args = list()
    kwargs = {}
    transform_train = transforms.Compose([
        transforms.Resize(32),
...
def make_medmnist_cnn(num_classes):
    act = torch.nn.ReLU
    return nn.Sequential(
...
class MEDMNIST_CNN:
    base = make_medmnist_cnn
    args = list()
    kwargs = {}


def make_uci_mlp(num_features):
    act = torch.nn.ReLU
    return nn.Sequential(
