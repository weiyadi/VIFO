# Modified from https://github.com/VectorInstitute/vbll/blob/main/vbll/layers/classification.py.
...
from posteriors.vbll_utils import Normal, DenseNormal, get_parameterization
...
class DiscClassification(nn.Module):
    ...
    def __init__(self,
        ...
        base_model = base(*args, **kwargs)
        self.base = torch.nn.Sequential(*list(base_model.children())[:-1])
        in_features = list(base_model.children())[-1].in_features
        out_features = list(base_model.children())[-1].out_features
